...
function Recommendations() {
  ...
  useEffect(() => {
    void Promise.all([getCatalog(), getLearningSignals(user.id)]).then(([catalog, signals]) => {
      ...
      setPreferences(signals.preferences)
      setEnrolled(signals.enrolled)
      setForm({
        subjectId: signals.preferences.firstSubjectId ?? '',
        topic: signals.preferences.topic ?? '',
        goal: signals.preferences.goal ?? ''
      })
    })
  }, [tick, user.id])
